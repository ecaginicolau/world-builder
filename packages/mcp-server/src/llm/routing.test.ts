...
import { pickTransport, type RoutingSettings } from "./routing.js";
...
describe("pickTransport", () => {
  ...
  it("falls back to cloud when settings is null", () => {
    const t = pickTransport(null, "extract", "cheapest");
    expect(t.mode.kind).toBe("cloud");
  });
  ...
  it("uses tier mapping when falling back to cloud", () => {
    expect(pickTransport(off, "extract", "cheapest").model).toBe("gpt-5.4-nano");
    expect(pickTransport(off, "upscale", "best").model).toBe("gpt-5.4");
  });
